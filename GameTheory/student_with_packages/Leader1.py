...
# Leader for follower 1, Profit of 17.63877 (5d.p)
...
class SimpleLeader(Leader):
    ...
    def calculate_daily_profit(self, leader_price):
        follower_price = np.exp(self.coefficient * leader_price + self.intercept)  # Transform back to the original scale
    ...
    def analyse_history(self, days=100):
        ...
        for day in range(1, days + 1):
            ...
            # Log transformation with safe guard against non-positive values
            if follower_price > 0:
                self.follower_prices.append(np.log(follower_price))
            else:
                self.follower_prices.append(-np.inf)  # Handling log(0) as negative infinity or consider small positive value
        ...
        # Store only the coefficients needed for an exponential regression
